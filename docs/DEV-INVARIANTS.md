...
| Table `clients` indexée par CONNEXION (`ws`), jamais par pseudo — test `multi-connexion.test.mjs` | `chat-module/src/server.js` | Une personne a plusieurs connexions : « Ajouter mon mobile » fait rejoindre le téléphone sous le MÊME pseudo. Indexée par pseudo, la seconde connexion écrasait la première : le poste restait ouvert mais sortait de toute rediffusion — sourd à tout, y compris aux autres, et même après le départ du téléphone. Corollaires : la présence dédoublonne les pseudos, et l'invitation ciblée sert TOUTES les connexions de la personne visée |
| Le filtre `extraResources` EXCLUT `tools/`, `test/` et les devDependencies (`esbuild`, `@esbuild`, `@noble`) — seul `node_modules/ws` est livré | `package.json` (`build.extraResources`) | Un `**/*` naïf expédiait 16 Mo d'outillage à chaque client, dont le binaire `esbuild.exe` ET `tools/make-licence.mjs`, l'émetteur de licences payantes dont l'en-tête affirme pourtant « jamais livré aux clients » |
